...
export default function MovieList({ navigation }) {
  ...
  useEffect(() => {
    async function getPickUpMovie() {
      try {
        const result = await axios.get(requests.movie.NOW_PLAYING);
  ...
  return (
    ...
    <TouchableOpacity onPress={() => navigation.navigate("MovieDetail", {movie: picupMovies})}>
      ...
      <MovieFlatList url={requests.movie.NOW_PLAYING} listName={'公開中の映画'} navigation={navigation}></MovieFlatList>
      <MovieFlatList url={requests.movie.COMMING_SOON} listName={'公開予定の映画'} navigation={navigation}></MovieFlatList>
      <MovieFlatList url={requests.movie.POPULARS} listName={'人気の映画'} navigation={navigation}></MovieFlatList>
      <MovieFlatList url={requests.movie.TOP_RATED} listName={'高評価の映画'} navigation={navigation}></MovieFlatList>
    </ScrollView>
  );
}
